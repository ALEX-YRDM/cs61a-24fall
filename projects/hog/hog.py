...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    tmp=take_turn(num_rolls,player_score,opponent_score,dice)+player_score
...
def play(strategy0, strategy1, update,
    ...
    while score0 <goal and score1<goal:
        if who==0:
            nums_roll = strategy0(score0,score1)
            score0 = update(nums_roll,score0,score1,dice)
        else:
            nums_roll = strategy1(score1,score0)
            score1 = update(nums_roll,score1,score0,dice)
        who = 1-who
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(goal):
        for score1 in range(goal):
            cur_roll = strategy(score0,score1)
            if first_roll is None:
                first_roll = cur_roll
            elif cur_roll!= first_roll:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=10000):
    ...
    tup = [0,0]
    for i in range(1,11):
        average_d = make_averaged(roll_dice,times_called)
        avg = average_d(i,dice)
        if avg > tup[1]:
            tup[0],tup[1]=i,avg
    return tup[0]
    # END PROBLEM 9
